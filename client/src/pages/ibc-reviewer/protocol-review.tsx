...
import type { IbcApplication } from "@shared/types";
...
export default function IbcProtocolReviewPage() {
  const [, params] = useRoute("/ibc-reviewer/protocol-review/:id");
  const [, setLocation] = useLocation();
  const applicationId = params?.id ? parseInt(params.id) : null;
  const [reviewComments, setReviewComments] = useState("");
  const [reviewDecision, setReviewDecision] = useState<"approve" | "request_changes" | "">("");
  ...
  const submitReviewMutation = useMutation({
    mutationFn: async (data: { comments: string; decision: string }) => {
    ...
    onSuccess: () => {
      toast({
        ...
        description: "Your review comments have been sent to the IBC office.",
      });
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {/* Review Decision */}
          <div>
            <label className="text-sm font-medium mb-3 block">Review Recommendation</label>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    <h4 className="font-medium">Recommend Approval</h4>
                    <p className="text-sm text-gray-500">Protocol meets all requirements</p>
                  </div>
                </div>
              </button>
              ...
              >
                <div className="flex items-center space-x-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-gray-500">Modifications needed before approval</p>
                  </div>
                </div>
              </button>
            </div>
          ...
          <div className="flex justify-end pt-4">
            <Button 
              ...
              disabled={submitReviewMutation.isPending || !reviewComments.trim() || !reviewDecision}
              className="min-w-32"
            >
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            <div>
              <h4 className="font-medium text-gray-900">Protocol Details</h4>
              <p className="text-gray-600">Number: {application.ibcNumber}</p>
              <p className="text-gray-600">Level: {application.biosafetyLevel}</p>
              <p className="text-gray-600">Status: {statusConfig?.label}</p>
            </div>
            <div>
              <h4 className="font-medium text-gray-900">Principal Investigator</h4>
              <p className="text-gray-600">{scientist?.name}</p>
              {scientist?.department && (
                <p className="text-gray-600">{scientist.department}</p>
              )}
              {scientist?.email && (
                <p className="text-gray-600">{scientist.email}</p>
              )}
            </div>
            <div>
              ...
              {application.vettedDate && (
                <p className="text-gray-600">
                  Vetted: {format(new Date(application.vettedDate), 'MMM d, yyyy')}
                </p>
              )}
              {application.underReviewDate && (
                <p className="text-gray-600">
                  Review Started: {format(new Date(application.underReviewDate), 'MMM d, yyyy')}
                </p>
              )}
            </div>
          </div>
